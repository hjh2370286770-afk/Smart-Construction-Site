...
新增：进出场逻辑 + 清洗判定 + MySQL数据库 + 实时播放 + 不生成视频
...
log_file = open('full_video_test_v3_realtime.log', 'w', encoding='utf-8')
...
class EntryExitManager:
    ...
    def _plate_similarity(self, p1: str, p2: str) -> float:
        """计算两个车牌的相似度 (使用统一缓存实现)"""
        from detectors.plate_utils import plate_similarity
...
log("=" * 70)
log("车辆清洗检测 V3 - 实时模式")
log("基于已验证SpatialPlateDeduplicator架构 + 进出场/清洗/数据库")
log("=" * 70)

# 视频源配置：文件或RTMP流
# VIDEO_PATH = r"D:\Users\Admini503\OneDrive\Desktop\文件\视觉识别\微信视频2026-04-23_090043_727.mp4"
VIDEO_PATH = "rtmp://rtmp05open.ys7.com:1935/v3/openlive/BK1948130_2_1?expire=1809742801&id=974617006789623808&t=f518ce2f69721d3ecb6e1e200cad4d2a81c4549dea9d8ee4c22a84c94809d3b8&ev=101"
...
try:
    ...
    if db_config.get('enabled', False):
        try:
            from detectors.vehicle_wash_db import VehicleWashDB
...
def plate_detection_thread():
    ...
    with ThreadPoolExecutor(max_workers=4) as executor:
        while not stop_event.is_set():
            ...
            result_queue.put(
                (idx, frame, plate_results, detect_time, plate_time),
                block=True
            )
...
def display_thread():
    """独立显示线程 - 不阻塞主处理流水线"""
    log("[Display] Started")
    window_name = "Vehicle Wash V3 (REALTIME)"
...
def result_processing_thread():
    ...
    while not stop_event.is_set():
        ...
        try:
            idx, frame, plate_results, detect_time, plate_time = result_queue.get(timeout=0.2)
        except queue.Empty:
        ...
        if not skip_drawing:
            ...
            for result in plate_results:
                ...
                if result['is_confirmed']:
                    ...
                    # 修复：颜色基于车牌文本哈希生成，保持稳定不闪烁
                    if plate_text not in tracked_confirmed:
                        # 使用车牌文本的哈希值生成稳定颜色
                        hash_val = hash(plate_text) % 0xFFFFFF
                        r = (hash_val >> 16) & 0xFF
                        g = (hash_val >> 8) & 0xFF
                        b = hash_val & 0xFF
                        # 确保颜色不太暗
                        ...
                        tracked_confirmed[plate_text] = color
                    else:
                        color = tracked_confirmed[plate_text]

                    record = entry_exit_mgr.records.get(plate_text)
                    if record:
                        if record.is_washed:
                            color = (0, 200, 255)
                    ...
                    cv2.putText(result_frame, det.class_name, (x1, y1 - 10),
