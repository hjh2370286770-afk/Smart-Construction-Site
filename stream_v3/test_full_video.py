...
try:
    ...
    from detectors.plate_validator_v2 import validate_plate_v2
    from detectors.vehicle_tracker import VehicleTracker
    log("  Import OK")
            ...
            for track in exited_tracks:
                if track.plate_text and track.plate_text in entry_exit_mgr.records:
                    ...
                    if record.exit_time is None:
                        ...
                        # 保存到数据库
                        if entry_exit_mgr.db:
                            try:
                                entry_exit_mgr.db.save_vehicle_record(record)
                                log(f"  数据库已更新出场: {track.plate_text}")
                            except Exception as e:
                                log(f"  数据库保存出场失败: {e}")
...
if confirmed_vehicles_detail:
    for i, vehicle in enumerate(confirmed_vehicles_detail, 1):
        ...
        # 获取车牌颜色和类型信息
        from detectors.plate_validator_v2 import PlateValidatorV2
