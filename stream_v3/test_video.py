...
try:
    ...
    detector_config = {
        ...
        'exit_line_y': 500,
        'save_images': True,
        'image_save_path': 'storage/test_video',
    }
...
# 创建输出视频
output_path = "storage/test_video/output.mp4"
Path("storage/test_video").mkdir(parents=True, exist_ok=True)
...
frame_idx = 0
process_every_n_frames = 3  # 每3帧处理一次，提高速度
start_time = time.time()
max_frames = min(frame_count, 300)  # 最多处理300帧（约20秒视频）
...
if completed:
    for record in completed[-5:]:  # 只显示最后5条
        status = "已清洗" if record.is_washed else "未清洗"
