...
# Detection settings - 车辆检测用降低的分辨率
DETECT_WIDTH = 1280
...
log(f"  Plate confidence threshold: 0.4")
...
try:
    ...
    from detectors.plate_utils import PlateDeduplicator, validate_and_filter_plate
    log("OK")
    ...
    if detector.plate_detector is not None:
        ...
        log("    - Using optimized plate recognition")
    else:
    ...
    log("    - Initialized smart plate deduplicator")
...
def plate_detection_worker(args):
    """
    Plate detection worker - uses original resolution for better accuracy
    
    Args:
        args: (original_frame, detection)
    """
    original_frame, detection = args
    
    x1, y1, x2, y2 = detection.bbox
    
    # 在原始分辨率图像上裁剪车辆区域（关键优化！）
    vehicle_roi = original_frame[y1:y2, x1:x2]
    
    if vehicle_roi.size == 0 or vehicle_roi.shape[0] < 20 or vehicle_roi.shape[1] < 50:
        return detection, None
    
    # 使用YOLOv8车牌检测器识别（在原始分辨率上）
    ...
    if plate_result and plate_result[0]:
        ...
        # 验证车牌格式
        is_valid, reason, cleaned = validate_and_filter_plate(plate_text)
        
        if is_valid and cleaned:
            return detection, (cleaned, plate_bbox, is_valid)
        else:
            return detection, None
    else:
        return detection, None
...
def plate_detection_thread():
    """Plate detection thread with validation and deduplication"""
    log("[Plate Detector] Started (with validation & deduplication)")
        ...
        while not stop_event.is_set():
            ...
            if detections:
                ...
                for future in as_completed(futures):
                    try:
                        det, plate_info = future.result(timeout=10.0)
                        
                        if plate_info:
                            cleaned_plate, plate_bbox, is_valid = plate_info
                            
                            if is_valid:
                                # 尝试添加到去重器
                                confirmed = deduplicator.add_plate(
                                    cleaned_plate, 
                                    current_timestamp,
                                    det.confidence
                                )
                                ...
                                if confirmed:
                                    plate_results.append((det, (confirmed, plate_bbox)))
                                    valid_count += 1
                                    
                                    with lock:
                                        if confirmed not in stats['confirmed_plates']:
                                            stats['confirmed_plates'].append(confirmed)
                                            log(f"[Plate ✓] New confirmed: {confirmed}")
                            else:
                                invalid_count += 1
                        else:
                            plate_results.append((det, None))
...
def result_processing_thread():
    ...
    while not stop_event.is_set():
        ...
        with lock:
            ...
            unique_plates = len(stats['confirmed_plates'])
...
with lock:
    ...
    valid_plates = stats['valid_plates']
    invalid_plates = stats['invalid_plates']
    confirmed_list = stats['confirmed_plates']
...
log(f"\nDetection Results:")
log(f"  Total vehicle detections: {total_detections}")
log(f"  Valid plates detected: {valid_plates}")
log(f"  Invalid plates filtered: {invalid_plates}")
log(f"  Validation rate: {valid_plates/(valid_plates+invalid_plates)*100:.1f}%" if (valid_plates+invalid_plates) > 0 else "  N/A")

log(f"\nFinal Confirmed Plates ({len(confirmed_list)}):")
if confirmed_list:
    for i, plate in enumerate(confirmed_list, 1):
        log(f"  {i}. {plate}")
else:
    log("  No plates confirmed")

log(f"\nDeduplicator Statistics:")
log(f"  Confirmed: {dedup_stats['confirmed_count']}")
log(f"  Candidates: {dedup_stats['candidate_count']}")
log(f"  Total candidates: {dedup_stats['total_candidates']}")
