...
# 添加 plate_recognition 模块路径 (相对 organized 目录)
sys.path.insert(0, str(Path(__file__).parent.parent / 'plate_recognition'))
...
class PlateDetector:
    ...
    def __init__(self, 
                 detect_model_path: str = 'models/yolov8s.pt',
                 rec_model_path: str = 'models/plate_rec_color.pth',
                 device: str = 'cpu',
                 img_size: int = 640,
        ...
        # 如果未指定路径，使用默认的相对 organized 目录路径
        if detect_model_path is None:
            detect_model_path = str(Path(__file__).parent.parent / 'models' / 'yolov8s.pt')
        if rec_model_path is None:
            rec_model_path = str(Path(__file__).parent.parent / 'models' / 'plate_rec_color.pth')
    ...
    def init(self) -> bool:
        ...
        try:
            ...
            from plate_rec import init_model
    ...
    def detect(self, image: np.ndarray) -> List[Dict]:
        ...
        try:
            from plate_rec import get_plate_result
